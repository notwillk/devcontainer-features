...
mkdir -p "$TMP_PROJECT/test/claude-auth-persist"

cat >"$TMP_PROJECT/test/claude-auth-persist/test.sh" <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
...
reportResults
EOS
chmod +x "$TMP_PROJECT/test/claude-auth-persist/test.sh"

devcontainer features test \
  --project-folder "$TMP_PROJECT" \
  --features claude-auth-persist \
